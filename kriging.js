...
Array.prototype.max = function() {
  ...
  export const kriging = (function() {
	...
	// Matrix algebra
	var krigingMatrixDiag = function(c, n) {
	  var i; var Z = [0].rep(n * n)
	  for (i = 0; i < n; i++) Z[i * n + i] = c
	  return Z
	}
	var krigingMatrixTranspose = function(X, n, m) {
	...
	// var krigingMatrixScale = function(X, c, n, m) {
	//   var i, j
	//   for (i = 0; i < n; i++) {
	//     for (j = 0; j < m; j++) { X[i * m + j] *= c }
	//   }
	// }
	var krigingMatrixAdd = function(X, Y, n, m) {
	...
	var krigingMatrixMultiply = function(X, Y, n, m, p) {
	...
	var krigingMatrixChol = function(X, n) {
	  var i; var j; var k; var p = Array(n)
	...
	var krigingMatrixChol2inv = function(X, n) {
	  var i, j, k, sum
	  for (i = 0; i < n; i++) {
		X[i * n + i] = 1 / X[i * n + i]
		for (j = i + 1; j < n; j++) {
	...
	var krigingMatrixSolve = function(X, n) {
	  var m = n
	  var b = Array(n * n)
	  var indxc = Array(n)
	  var indxr = Array(n)
	...
	// Variogram models
	var krigingVariogramGaussian = function(h, nugget, range, sill, A) {
	  return nugget + ((sill - nugget) / range) *
		(1.0 - Math.exp(-(1.0 / A) * Math.pow(h / range, 2)))
	}
	var krigingVariogramExponential = function(h, nugget, range, sill, A) {
	  return nugget + ((sill - nugget) / range) *
		(1.0 - Math.exp(-(1.0 / A) * (h / range)))
	}
	var krigingVariogramSpherical = function(h, nugget, range, sill, A) {
	...
	kriging_.train = function(t, x, y, model, sigma2, alpha) {
	  var variogram = {
		t: t,
		x: x,
		y: y,
	  ...
	  if (krigingMatrixChol(Z, 2)) { krigingMatrixChol2inv(Z, 2) } else {
	  ...
	  if (krigingMatrixChol(C, n)) { krigingMatrixChol2inv(C, n) } else {
	  ...
	  var Ks = C.slice(0)
	  var M = krigingMatrixMultiply(C, t, n, n, 1)
	  variogram.K = Ks
	  variogram.M = M
	...
	// Model prediction
	kriging_.predict = function(x, y, variogram) {
	...
	kriging_.variance = function(x, y, variogram) {
	  var i; var k = Array(variogram.n)
	  for (i = 0; i < variogram.n; i++) {
		k[i] = variogram.model(Math.pow(Math.pow(x - variogram.x[i], 2) +
		  Math.pow(y - variogram.y[i], 2), 0.5),
	  ...
	  return variogram.model(0, variogram.nugget, variogram.range,
		variogram.sill, variogram.A) +
		krigingMatrixMultiply(krigingMatrixMultiply(k, variogram.K,
		  1, variogram.n, variogram.n),
		k, 1, variogram.n, 1)[0]
	}
  
	// Gridded matrices or contour paths
	kriging_.grid = function(polygons, variogram, width) {
		...
		for (j = a[0]; j <= a[1]; j++) {
		  for (k = b[0]; k <= b[1]; k++) {
			...
			if (polygons[i].pip(xtarget, ytarget)) {
			  A[j][k] = kriging_.predict(xtarget,
				ytarget,
				variogram)
			}
		  }
	...
	kriging_.contour = function(value, polygons, variogram) {
  
	}
  
	// Plotting on the DOM
	kriging_.plot = function(canvas, grid, xlim, ylim, colors) {
	  // Clear screen
