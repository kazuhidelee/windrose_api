...
<head>
  ...
  <script src="https://unpkg.com/@sakitam-gis/kriging/dist/kriging.min.js" type="text/javascript"></script>
...
<div id="content">
    ...
    for (i = 0, k = false; i < n; i++, k = false) {
      ...
      while (k == false) {
        ...
        for (j = 0; j < world.length; j++) {
          if (kriging.pip(world[j], x[i], y[i])) {
            k = true;
            break;
          }
        }
  ...
  <div id="start"></div>
  <p><strong>kriging.js</strong> is a Javascript library for mapping and predicting spatial data using the <a
    href="http://en.wikipedia.org/wiki/Kriging" target="_newtab">kriging algorithm</a>. This library allows one to model
    and map spatial data.</p>
  <p>The easiest way to get started is to <a href="http://rawgithub.com/oeo4b/kriging.js/master/kriging.js"
                                             download="kriging.js">download</a> the Javascript library (or link directly
    to krigingjs.com/kriging.js) and attach it to your html code. Then, to train a <a href="#documentation">model</a>
    with coordinates (<strong>x</strong>, <strong>y</strong>) and values <strong>t</strong> on <a href="#documentation">parameters</a>
    &sigma;<sup>2</sup> and &alpha;:</p>
...
var fitModel = kriging.train(t, x, y, model, sigma2, alpha);</pre>
  ...
  <ul>
    ...
    <li>
      ...
      <p>The model used is a 2-dimensional gaussian process, <b>y</b>, with a prior on the parameters of the kernel
        function. One of a variety of different kernel functions (or variogram models) generate the Gram matrix <b>K</b>.
        The variance &sigma;<sup>2</sup> of the noise process is adjustable, providing a smoothing effect by increasing
        the uncertainty of the likelihood <b>t</b>.</p>
      <pre><b>y</b>&nbsp;&nbsp; ~ N(<b>0</b>, <b>K</b>)
<b>t</b>|<b>y</b> ~ N(<b>t</b>|<b>y</b>, &sigma;<sup>2</sup><b>I</b>)</pre>
    </li>
    <li>
      <h3>Variogram</h3>
      <p>The various variogram models can be interpreted as kernel functions for 2-dimensional coordinates
        <strong>a</strong>, <strong>b</strong> and parameters nugget, range, sill and A. Reparameterized as a linear
        function, with w = [nugget, (sill-nugget)/range], this becomes:</p>
      <pre>
...
Spherical: k(<strong>a</strong>,<strong>b</strong>) = w[0] + w[1] * ( 1.5 * ( ||<strong>a</strong>-<strong>b</strong>|| / range ) - 0.5 * ( ||<strong>a</strong>-<strong>b</strong>|| / range )<sup>3</sup> )</pre>
  ...
  <p></p>
  <div id="footer">
    <small>Copyright 2011-2015, Omar Olmedo. Released under <a href="http://opensource.org/licenses/MIT">The MIT
      License</a>.
    </small>
  </div>
</div>
